...
run_add_site() { # sets SEO_AUTOPILOT_HOME, LAUNCH_AGENTS_DIR; runs add-site.sh demo
  setup_mocks
  SEO_AUTOPILOT_HOME="$(mktemp -d)"; export SEO_AUTOPILOT_HOME
  mkdir -p "$SEO_AUTOPILOT_HOME/sites" "$SEO_AUTOPILOT_HOME/logs" "$SEO_AUTOPILOT_HOME/bin"
...
( # Fresh setup: a missing REPO_PATH is cloned automatically
  ...
  AS_DOMAIN="demo.com" AS_REPO_PATH="$clone_path" AS_REPO_URL="$source_repo" \
  ...
  assert_eq "$(git -C "$clone_path" remote get-url origin)" "$source_repo" "fresh clone records the requested origin"
